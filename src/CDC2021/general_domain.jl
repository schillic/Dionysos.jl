...
function GeneralDomainList(hx,elems=Set{NTuple{length(hx),Int}}();periodic=Int[],periods=Float64[],T0=zeros(length(periodic)),lims=nothing,fit=false,f=nothing,fi=nothing)
    N = length(hx)
    x0 = zeros(N)
    ...
    if f!=nothing
        grid = AB.DeformedGrid(grid, f, fi)
    end
